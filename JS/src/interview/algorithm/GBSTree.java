...
public class GBSTree<T extends Comparable> {
	...
	public static void main(String[] args) {
		...
		GBSTree<String> t = new GBSTree<String>();
		...
		System.out.println();
		System.out.println("getPathBetweenTwoNodes a & e");
		t.getPathBetweenTwoNodes(t.getRoot(), "a", "e");
		System.out.println();
	}
	...
	public void getPathToRoot(Node<T> node, LinkedList<Node<T>> path) {
		while (null!=node) {
			path.add(node);
			node = node.getParent();
		}
	...
	public void getPathFromRoot(Node<T> node, LinkedList<Node<T>> path) {
		if (null!=node && null!=node.getParent()) {
	...
	private StringBuffer mergePaths(LinkedList<Node<T>> rootToVal1, LinkedList<Node<T>> rootToVal2) {
		StringBuffer result = new StringBuffer();
		Node<T> lastDivergence = null;
		while (rootToVal1.peekFirst() == rootToVal2.peekFirst()) {
			lastDivergence = rootToVal1.pollFirst();
			rootToVal2.pollFirst();
		}
		
		Iterator<Node<T>> path1Iter = rootToVal1.iterator();
		Iterator<Node<T>> reversedPath2Iter = rootToVal2.descendingIterator();
		
		while(reversedPath2Iter.hasNext()) {
			result.append(reversedPath2Iter.next().data.toString()+", ");
		}
		result.append(lastDivergence.data.toString()+", ");
		while(path1Iter.hasNext()) {
			result.append(path1Iter.next().data.toString()+", ");
		}
		return result;
	}
	...
	public void getPathBetweenTwoNodes(Node<T> root, T val1, T val2) {
		...
		LinkedList<Node<T>> rootToVal1 = new LinkedList<Node<T>>();
		getPathFromRoot(node1, rootToVal1);
		LinkedList<Node<T>> rootToVal2 = new LinkedList<Node<T>>();
